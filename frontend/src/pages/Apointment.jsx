...
function Apointment() {
  ...
  const { doctors, currencySymbol, getDoctorData, backendUrl, token } =
  ...
  const getAvailableSlots = async () => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      while (currentDate < endTime) {
        let formetteTime = currentDate.toLocaleTimeString([], {
          hour: "2-digit",
          minute: "2-digit",
        });
        ...
        const isSlotAvailable =
          docData &&
          docData.slots_book[slotDate] &&
          docData.slots_book[slotDate].includes(slotTime)
            ? false
            : true;
        if (isSlotAvailable) {
          //add slot to array
    ...
    <div>
      ...
      <div className="flex flex-col md:flex-row gap-4">
        ...
        <div className="flex-1 border border-gray-400 p-8 rounded-lg py-7 bg-white mx-2 sm:mx-0  sm:mt-0 ">
          ...
          <div className="flex items-center gap-2 text-sm text-gray-600 mt-1">
            ...
            <button className="py-0.5 px-2 rounded-full bg-primary text-xs text-white">
              {docData?.experience}
            </button>
          </div>
          {/* Doctor About */}
          <div>
            <p className="flex items-center gap-2 text-sm text-gray-900 font-medium mt-3">
              About <img className="w-3.5" src={assets.info_icon} alt="" />
            </p>
            <p className="text-sm text-gray-800 font-medium max-w-[700px] mt-2">
              {docData?.about}
            </p>
          </div>
          {/* Appointment fess */}
      ...
      <div className="sm:ml-72 sm:pl-14 mt-4 font-medium text-gray-700">
          ...
          {docSlots.length &&
            docSlots.map((item, index) => (
              <div
                ...
                className={`text-center py-6 px-2 rounded-full min-w-16 cursor-pointer ${
                  slotIndex === index
        ...
        <button
          ...
          className="bg-primary text-white text-sm py-3 px-14 mt-4 rounded-full hover:scale-105 transition-all"
        >
          Book Appointment
        </button>
      </div>
