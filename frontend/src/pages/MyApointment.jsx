...
function MyApointment() {
  ...
  const makePayment = async (appointmentId) => {
    try {
      const { data } = await axios.post(
        `${backendUrl}/api/v1/user/get-amount`,
        { appointmentId }
      );
      if (data.success) {
        await axios.post(`${backendUrl}/api/v1/user/change-payment-status`, {
          appointmentId,
        });
        toast.success("payment change");
        console.log(data.appointmentData);
        navigate("/payment", {
          state: { appointmentId, amount: data.appointmentData.amount },
        });
      } else {
        toast.error(data.message);
      }
  ...
  const deleteAppointment = async (appointmentId) => {
    try {
      const { data } = await axios.post(
        `${backendUrl}/api/v1/user/delete-appointment`,
        { appointmentId }
      );
      if (data.success) {
        ...
        {appointments.slice(0, 8).map((item, index) => (
          ...
          >
            ...
            <div className="flex-1 text-sm text-zinc-600">
              ...
              <p className="mt-1">
                <span className="text-neutral-700 font-semibold mt-1">
                  Date and Time:{" "}
                </span>{" "}
            ...
            <div className="flex flex-col gap-2 justify-center">
              {!item.cancelled && !item.payment && !item.isCompleted && (
                <button
                  ...
                  className="text-sm text-stone-700 text-center sm:min-w-48 py-2 border rounded hover:bg-primary hover:text-white transition-all duration-200"
                >
                  Pay Online
                </button>
              )}
              {!item.cancelled && !item.isCompleted && (
                <button
                  ...
                  className="text-sm text-stone-700 text-center sm:min-w-48 py-2 border rounded hover:bg-red-700 hover:text-white transition-all duration-200"
                >
                  Cancel Appointment
                </button>
              )}
