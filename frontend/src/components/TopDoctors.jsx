...
const TopDoctors = () => {
  ...
  return (
    <div className='flex flex-col gap-4 items-center my-16 text-gray-900 md:mx-10'>
      ...
      <div className='w-full grid grid-cols-auto gap-4 pt-5 sm:px-0 px-3 gap-y-6'>
        {
            doctors.slice(0,10).map((item)=>(
                <div onClick={()=>navigate(`/apointment/${item._id}`)} className='border border-blue-200 rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] transition-all duration-300' key={item._id}>
      ...
      <button onClick={()=>navigate('/doctors')} className='bg-blue-50 text-gray-800 px-12 py-4 mt-4 rounded-full hover:bg-primary hover:text-white'>More</button>
    </div>
  )
}
